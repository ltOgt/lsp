/// Taken from [1] with small fix
/// [1] github.com/natebosch/dart_lsp/blob/d17deb45de46c3ffd8fe87b5fc0c7456228c2004/lib/src/protocol/language_server/wireformat.dart
import 'dart:async';
import 'dart:convert';
...
StreamChannel<String> lspChannel(Stream<List<int>> stream, StreamSink<List<int>> sink) {
...
class _Parser {
  ...
  _Parser(Stream<List<int>> stream) {
    _subscription = stream.expand((bytes) => bytes).listen(_handleByte, onDone: () {
      _streamCtl.close();
    });
  }
  ...
  int _parseContentLength() {
    ...
    final lengthHeader = headers.firstWhere((h) => h.startsWith('Content-Length'));
  ...
  bool get _headerComplete {
    ...
    return l > 4 && _buffer[l - 1] == 10 && _buffer[l - 2] == 13 && _buffer[l - 3] == 10 && _buffer[l - 4] == 13;
  }
}
